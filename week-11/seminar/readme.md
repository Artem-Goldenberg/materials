...
которые происходят в нашей системе, записывать в некоторый лог операций, который будет
...
В ней описывается древнегреческий остров Паксос (Paxos), где 
существует парламент, которому необходимо принимать некоторые решения. Всё усложняется тем,
...
Традиционно алгоритм Paxos позволяет нескольким узлам договориться о значении одной переменной.
...
Алгоритм предлагает ввести две роли в систему: есть Proposer'ы &mdash; узлы, которые предлагают 
значение, и Acceptor'ы &mdash; узлы, которые принимают предложения от Proposer'ов. 
Один узел может одновременно реализовывать две роли: предлагать значения и участвовать в их принятии.

Исполнение алгоритма разделяется на логические раунды.
В каждом раунде Proposer'ы обращается к Acceptor'ам и начинают двухэтапный процесс предложения 
...
В 1985 году учёные Michael J. **F**ischer, Nancy A. **L**ynch и Michael S. **P**aterson публикуют статью 
"[Impossibility of Distributed Consensus with One Faulty Process](https://groups.csail.mit.edu/tds/papers/Lynch/jacm85.pdf)",
основной результат которой часто называют FLP-теоремой по первым буквам фамилий авторов.

В рамках FLP-теоремы показывается, что в асинхронной распределенной системе нельзя гарантировать достижение консенсуса (свойство liveness) в присутствии хотя бы одного crash-отказа узла. Это утверждение справедливо для Paxos и для всех других алгоритмов консенсуса. 
...
Также доступна более подробная [диссертация](https://github.com/ongardie/dissertation#readme)
 Diego Ongaro, в рамках которой и раскрывается идея алгоритма.
...
Примерно в то же время, что и публикация статьи, появились Docker и Kubernetes, начал своё
...
Внутри ZooKeeper с помощью алгоритма [Zab](https://www.researchgate.net/profile/Benjamin-Reed-9/publication/224248727_Zab_High-performance_broadcast_for_primary-backup_systems/links/640eb61b66f8522c389d32e9/Zab-High-performance-broadcast-for-primary-backup-systems.pdf) реализован примитив 
