...
В этой лабе мы сравним несколько вариантов реализации gossip с помощью небольшого симулятора на уже знакомом вам фреймворке [AnySystem](https://github.com/systems-group/anysystem).

## Подготовка

Откройте репозиторий курса в IDE. Если у вас не VS Code, добавьте `anysystem/python` в `PYTHONPATH`.

Если у вас установлен Rust, скомпилируйте симулятор с помощью команды `cargo install --path simulator` и попробуйте запустить `distsys-gossip -h`. Вы должны увидеть справку по запуску.

Если вы предпочитаете воспользоваться Docker-образом, замените выше и далее `distsys-gossip` на  `docker run --rm -t -v ./:/impl ghcr.io/distsys-course/gossip:latest`.
...
```
$ distsys-gossip -i push.py -n 1000 -f 2 -q
...
```
$ distsys-gossip -i pull.py -n 1000 -f 2
...
```
$ distsys-gossip -i push_pull.py -n 1000 -f 2 -q
...
```
$ distsys-gossip -i push_pull_stop.py -n 1000 -f 2
