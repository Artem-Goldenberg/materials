...
```
### Diffie-Hellman
RSA и другие подобные криптосистемы работают на практике, но симметричная криптография всё-таки заметно быстрее. Поэтому асимметричная криптография часто используется только для выработки общего секретного ключа, после чего стороны переходят на симметричные алгоритмы. Протокол Диффи-Хеллмана был исторически первым протоколом выработки общего ключа, и он до сих пор используется.

В начале стороны выбирают случайное большое простое число $p$ и ненулевое число $g < p$. После этого Алиса выбирает случайный ненулевой остаток $a$ по модулю $p$, а Боб аналогичным образом выбирает свой остаток $b$. Стороны обмениваются числами $g^a \mod p$ и $g^b \mod p$, после чего каждая сторона получает общий секрет $g^{ab} \mod p$. Из этого секрета стороны получают один или несколько ключей с помощью специальной функции (KDF).
...
Для аутентификации и авторизации на уровне приложения можно использовать токены. Клиент отправляет вместе с запросом выданный ему ранее токен, который подтверждает личность клиента и возможно содержит дополнительную информацию его правах. Распространённый формат токенов -- [JSON Web Token (JWT)](https://jwt.io/). JWT-токен состоит из заголовка, тела и подписи (см. картинку). Заголовок и тело -- обычные JSON-словари. Тело токена содержит необходимую приложению информацию о личности и правах владельца токена. Чтобы пользователь не мог сам отредактировать свой токен и прописать себе недостающие права, каждый токен подписывается приложением. Для передачи токена внутри URL заголовок, тело и подпись кодируются в base64 и разделяются точкой.

![JWT Token](img/jwt.png)

Есть [море библиотек](https://jwt.io/libraries) для работы с JWT в разных языках программирования. Пример работы с JWT в питоне можно посмотреть [здесь](./jwt).
