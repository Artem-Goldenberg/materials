...
Теперь разберемся как запускать и тестировать нашу систему. Тесты и сам фреймворк написаны на языке Rust, а Python мы будем использовать только для описания логики процессов. Поэтому необходимо [установить Rust](https://www.rust-lang.org/tools/install) и, разумеется, Python. Вместо установки Rust и компиляции тестов можно воспользоваться готовым Docker-образом, но в таком случае вы не сможете изменять код тестов.
