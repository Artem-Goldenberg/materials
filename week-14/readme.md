...
Идею византийских отказов Leslie Lamport хорошо раскрывает в статье
...
Например, если добавить Raft византийские отказы, выбранный лидер может успешно отвечать на пробы
...
В статье Лэмпорт доказывает, что если n < 3f + 1, то задача не решается.
...
другим двум узлам разные команды. Когда другие узлы захотят обменяться между собой команды, они увидят,
...
*Источник: The Byzantine Generals Problem, Leslie Lamport*

### Practical Byzantine Fault Tolerance

Алгоритм [Viewstamped Replication](https://pmg.csail.mit.edu/papers/vr-revisited.pdf) решает задачу
консенсуса в условиях crash-отказов. Идея заключается в выделении явного лидера и бэкап-реплик.
...
*Источник: Viewstamped Replication Revisited, Barbara Liskov and James Cowling*
...
с византийскими отказами при n > 3f. 

![](media/pbft-stages.png)

*Источник: Practical Byzantine Fault Tolerance, Miguel Castro and Barbara Liskov*

В рамках pBFT добавляется дополнительная стадия. Причем теперь реплики транслируют сообщение
все другим репликам. И каждая новая стадия начинается, только когда реплика получает сообщение
от 2f + 1 реплик с предыдущих стадий. Это необходимо, так как среди 2f + 1 реплик всегда найдется как
минимум f + 1 не отказавшая реплика, а значит среди 2f + 1 значений найдется значение, которое
встречается как минимум f + 1 раз и ему можно "доверять".

По итогу ожидается, что после commit-фазы как минимум f + 1 корректная реплика ответит клиенту,
...
*new_leader = epoch MOD replicas_number*.

Подробнее со стадиями работы алгоритма можно ознакомиться в 
[Секции 4](https://pmg.csail.mit.edu/papers/osdi99.pdf).

### Основные выводы

В условиях наличия византийских отказов
- Leslie Lamport показал, что при n < 3f + 1 консенсус не достижим;
- алгоритм pBFT решает задачу при n = 3f + 1 и более.
...
является адресом кошелька, известного всем участникам системы, довольно легко можно проверить, что
